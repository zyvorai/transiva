...
package_bundle_client_scripts() {
    ...
    for src in package-install.sh package-client-install.sh package-client-test.sh; do
        if [ ! -f "${lib}/${src}" ]; then
            echo "ERROR: missing ${lib}/${src} — cannot build user tarball" >&2
            return 1
        fi
    done
    ...
    package_bundle_user_docs "${stage}" "${build_dir}" "${product}" "${kind}"
}

# HELP.txt, START_HERE.txt, ZYVOR_INSTALL.txt
package_bundle_user_docs() {
    local stage="$1" build_dir="$2" product="$3" kind="${4:-minimal}"
    local lib="${build_dir}/scripts/lib"
    if [[ -x "${lib}/write-user-help.sh" ]]; then
        "${lib}/write-user-help.sh" "${stage}" "${product}" "${kind}"
    else
        echo "WARN: missing ${lib}/write-user-help.sh" >&2
    fi
    ...
    package_bundle_user_pdfs "${stage}" "${build_dir}" "${product}"
}

# Branded PDF copies of user .txt docs → docs/pdf/ + docs/zyvor-logo.png
package_bundle_user_pdfs() {
    local stage="$1" build_dir="$2" product="$3"
    local version="${4:-${V9S_PACKAGE_VERSION:-latest}}"
    local lib="${build_dir}/scripts/lib"
    if [[ ! -x "${lib}/generate-user-pdfs.sh" ]]; then
        echo "WARN: missing ${lib}/generate-user-pdfs.sh — skipping PDF docs" >&2
        return 0
    fi
    chmod +x "${lib}/generate-user-pdfs.sh"
